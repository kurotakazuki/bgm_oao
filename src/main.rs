...
fn main() -> Result<()> {
    let bubs = vec![
        ...
        // ),
        ("bass", "Bass", "0 0 0 y<0 0.45 0 0 0 0<=y 0.55"),
        ...
        ("kick", "Kick", "0 0 0 z<0 0.55 0 0 0 0<=z 0.45"),
        ("kick_last", "Kick Last", "0 0 0 z<0 0.55 0 0 0 0<=z 0.45"),
        ("mini_bass", "Mini Bass", "0 0 0 0==0 0.5*sin(Z-N/S)+0.5"),
        ...
        (
            "phan2_last",
            "Phan2 Last",
            "0 0 0 0==0 0.5*cos(Z+5*N/S)+0.5",
        ),
        (
            "pluck_chord_last",
            "Pluck Chord Last",
            "0 0 0 0==0 0.5*cos(Y+8*N/S)+0.5",
        ),
        (
            "sine_chord",
            "Sine Chord",
            "0 0 0 0==0 0.25*cos(Z+8*N/S)+0.75",
        ),
        ...
        ("stereo_guitar.L", "Guitar L", "0 0 0 y<0 1"),
        ("stereo_guitar.R", "Guitar R", "0 0 0 0<y 1"),
        ("stereo_pha-n.L", "Pha-n L", "0 0 0 0==0 0.5*cos(X+N/S)+0.5"),
        ("stereo_pha-n.R", "Pha-n R", "0 0 0 0==0 0.5*cos(X-N/S)+0.5"),
        ("stereo_phan.L", "Phan L", "0 0 0 0==0 0.5*cos(Z-N/S)+0.5"),
        ("stereo_phan.R", "Phan R", "0 0 0 0==0 0.5*cos(Z+N/S)+0.5"),
        ("stereo_phan2.L", "Phan2 L", "0 0 0 0==0 0.5*cos(Y-N/S)+0.5"),
        ("stereo_phan2.R", "Phan2 R", "0 0 0 0==0 0.5*cos(Y+N/S)+0.5"),
        (
            "stereo_pluck_chord.L",
            "Pluck Chord L",
            "0 0 0 Z<0 1.5/(0.5*PI)*E^(-(x^2+y^2)/0.5)",
        ),
        (
            "stereo_pluck_chord.R",
            "Pluck Chord R",
            "0 0 0 0<Z 1.5/(0.5*PI)*E^(-(x^2+y^2)/0.5)",
        ),
        ("stereo_po.L", "Po L", "0 0 0 0==0 0.5*cos(X-N/S)+0.5"),
        ("stereo_po.R", "Po R", "0 0 0 0==0 0.5*cos(X-N/S+PI)+0.5"),
        ("stereo_toro.L", "Toro L", "0 0 0 0==0 0.5*cos(Y-N/S)+0.5"),
        (
            "stereo_toro.R",
            "Toro R",
            "0 0 0 0==0 0.5*cos(Y-N/S+PI)+0.5",
        ),
        ("sub_bass1", "Sub Bass1", "0 0 0 x<0 0.48 0 0 0 0<=x 0.52"),
        ("sub_bass2", "Sub Bass2", "0 0 0 z<0 0.52 0 0 0 0<=z 0.48"),
        (
            "xylophone",
            "Xylophone",
